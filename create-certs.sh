...
for f in ca-root.crt command-handler.key command-handler-signed.pem
do
    docker cp ${f} dummy:/root/
done

for i in kafka-1 kafka-2 kafka-3 command-handler command-generator heartbeat graphql-endpoint
do
    echo ${i}
    # Create keystores
    ...
    for f in kafka.${i}.keystore.jks kafka.${i}.truststore.jks
        do
        docker cp ${f} dummy:/root/
    done
    ...
    then
        ...
        for f in ${i}_sslkey_creds ${i}_keystore_creds ${i}_truststore_creds
            do
            docker cp ${f} dummy:/root/
        done
    fi
